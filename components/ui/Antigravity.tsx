...
function AntigravityInner({
    ...
    depthFactor = 1,
    pulseSpeed = 3,
    particleShape = 'capsule' as ParticleShape,
    fieldStrength = 10,
}: AntigravityProps) {
    ...
    // Track mouse at window level so particles respond even under overlapping elements
    ...
    useEffect(() => {
        ...
        const onMove = (e: MouseEvent) => {
            ...
            windowPointer.current.x = ((e.clientX - rect.left) / rect.width) * 2 - 1;
            windowPointer.current.y = -((e.clientY - rect.top) / rect.height) * 2 + 1;
        };
...
export default function Antigravity({ className, style, ...props }: AntigravityProps) {
    return (
        <Canvas
