  ...
  useEffect(() => {
    ...
    try {
      renderer = new THREE.WebGLRenderer({
        ...
        precision: settings.precision as THREE.TextureDataType extends never ? never : string,
        stencil: false,
        depth: false,
      });
    } catch {
    ...
    // Cap at 24 fps — it's a slow ambient background effect, not UI
    const frameTime = 1000 / 24;

    const animate = (currentTime: number) => {
      if (!materialRef.current || !rendererRef.current || !sceneRef.current || !cameraRef.current) return;
      // Only render when the footer is actually visible on screen
    ...
    // IntersectionObserver — pause rendering when footer is off-screen
    const observer = new IntersectionObserver(
      entries => {
        isVisibleRef.current = entries[0].isIntersecting;
      },
      // Start rendering slightly before it enters view for a smooth entry
